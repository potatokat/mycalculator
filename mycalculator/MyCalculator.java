...
public class MyCalculator {
	...
	public int nfactorial(int n){
		int factorial = 1;
		if(n >= 0 && n < 99){	// assume 99 is the greatest possible number that a factorial can be generated
			for(int i=1;i<n;i++)
				factorial *= i;
			
			return factorial;
		}else{			// returns -1 if n is a negative number or an extremely large number
			return (-1);
		}
	}
